...
def predict(Ge, Al, OH, H2O, F, OSDA, B, Na2O, Cl, Temperature, time, AR, Area, CN, rpm):
    features = np.array([[Ge, Al, OH, H2O, F, OSDA, B, Na2O, Cl, Temperature, time, AR, Area, CN, rpm]], dtype=float)
...
def main():
    st.title("Extra-Large Pore Zeolite Prediction")
    html_temp = """
    ...
    <h1 style ="color:black;text-align:center; font-size: 10px;">
     ...
     (5) Prediction- If MR present in zeolite: [1]- Extra-large pore zeolite, [0] - small or medium or large pore zeolite</h1>
    </div>
    """
    ...
    if st.button("Predict"):
        result = predict(Ge, Al, OH, H2O, F, OSDA, B, Na2O, Cl, Temperature, time, AR, Area, CN, rpm)
